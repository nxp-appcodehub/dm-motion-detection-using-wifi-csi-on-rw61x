...
static lv_obj_t *auth_label, *auth_img, *g_label;

static uint8_t InputNumCnt;
...
static void PWDNumKeyEventCbk(lv_event_t *event)
{
	...
	{
		uint16_t keyIndex = lv_btnmatrix_get_selected_btn(inputNumKey);
		PRINTF("keyIndex = %d\r\n", keyIndex);
		if (keyIndex < 10)
		{
			InputNumCnt++;
			...
			lv_textarea_add_char(inputTextarea, *lv_btnmatrix_get_btn_text(inputNumKey, keyIndex));
		}
		else if (keyIndex == 10)
		{
			char *str = (char *)lv_textarea_get_text(inputTextarea);
			uint32_t pwd = atol(str);
			if (pwd == PWD_OK && InputNumCnt == PWDLENGTH)
			{
				//Right Pin code
				PRINTF("Correct pin code\r\n");
				lv_obj_del(PWDInputPage);
				PWDInputPage = NULL;
				...
				auth_label = lv_label_create(screen);
				lv_style_set_text_color(&style_title, lv_color_hex(0x006400));
				lv_obj_add_style(auth_label, &style_title, 0);
				lv_obj_align(auth_label, LV_ALIGN_TOP_MID, 0, 50);
				lv_label_set_text(auth_label, "Congratulation!");

				auth_img = lv_img_create(lv_scr_act());
				LV_IMG_DECLARE(lock_open);
				lv_img_set_src(auth_img, &lock_open);
				lv_obj_center(auth_img);
			}
			else
			{
				//Wrong Pin code
				...
				auth_label = lv_label_create(screen);
				lv_style_set_text_color(&style_title, lv_color_hex(0xffff00));
				lv_obj_add_style(auth_label, &style_title, 0);
				lv_obj_align(auth_label, LV_ALIGN_TOP_MID, 0, 50);
				lv_label_set_text(auth_label, "Auth Failed!");

				auth_img = lv_img_create(lv_scr_act());
				LV_IMG_DECLARE(lock_close);
				lv_img_set_src(auth_img, &lock_close);
				lv_obj_center(auth_img);
			}
		}
		else
		{
			...
			lv_textarea_del_char(inputTextarea);
		}
	}
}
...
void lv_demo_widgets(void)
{
    ...
    lv_style_set_text_font(&style_keyboard, &lv_font_montserrat_16);
    ...
    //lv_obj_add_style(label, &style_title, 0);
    //lv_label_set_text(label, "Motion Detected!");
    ...
    LV_IMG_DECLARE(motion_detect_1);
    lv_img_set_src(img, &motion_detect_1);
    lv_obj_center(img);
#endif
...
#if 0
    {
    	...
    	lv_obj_add_style(kb, &style_keyboard, 0);

    	/*Create a text area. The keyboard will write here*/
    	lv_obj_t * ta;
    	ta = lv_textarea_create(lv_scr_act());
    	lv_obj_align(ta, LV_ALIGN_TOP_LEFT, 10, 10);
    	lv_obj_add_event_cb(ta, kb_event_cb, LV_EVENT_ALL, kb);
...

    {
    	...
    	lv_obj_clear_flag(centerCont, LV_OBJ_FLAG_SCROLLABLE);
    	lv_obj_add_event_cb(centerCont, PassWordInputPageEventCbk, LV_EVENT_ALL, NULL);

    	lv_obj_t *tipLabel = lv_label_create(centerCont);
    	lv_obj_align(tipLabel, LV_ALIGN_TOP_LEFT, 0, 0);
    	lv_obj_add_style(tipLabel, &style_label, 0);
    	lv_label_set_text_static(tipLabel, "Please input 4-digital pin code");
    	...
    	static lv_point_t linePoints[2] = { {0, 18}, {220, 18} };
    	...
    	lv_textarea_set_password_mode(inputTextarea, true);
    	...
    	lv_obj_add_style(inputNumKey, &style_keyboard, 0);
...
void lv_demo_widgets_input_pincode(void)
{
	PRINTF("IP[0]\r\n");
    ...
    lv_obj_set_style_border_side(centerCont, LV_BORDER_SIDE_NONE, 0);
    lv_obj_set_style_bg_color(centerCont, lv_color_hex(0xebebeb), 0);
    lv_obj_align(centerCont, LV_ALIGN_CENTER, 0, 0);
    lv_obj_clear_flag(centerCont, LV_OBJ_FLAG_SCROLLABLE);
    lv_obj_add_event_cb(centerCont, PassWordInputPageEventCbk, LV_EVENT_ALL, NULL);

    lv_obj_t *tipLabel = lv_label_create(centerCont);
    //lv_obj_align(tipLabel, LV_ALIGN_TOP_LEFT, 0, 0);
    lv_obj_align(tipLabel, LV_ALIGN_TOP_MID, 0, 0);
    lv_obj_add_style(tipLabel, &style_label, 0);
    lv_obj_set_style_text_color(tipLabel, lv_color_hex(0x000000), 0);
    lv_label_set_text_static(tipLabel, "Please input 4-digit pin code");
...
extern void Demo_Display_Enable(bool enable);
void lv_demo_widgets_display(uint8_t event)
{
    ...
    switch(event)
    {
        case DIS_EVENT_SENSING:
            //lv_label_set_text(label, "Sensing!");
            //lv_style_set_text_color(&style_title, lv_color_hex(0xff0000));
            //lv_style_set_text_opa(&style_text_muted, LV_OPA_TRANSP);
        	if (g_label)
        	{
        		lv_obj_del(g_label);
        		g_label = NULL;
        	...
        	Demo_Display_Enable(false);
            break;
        case DIS_EVENT_NEAR_DECT:
            //lv_label_set_text(label, "Near Detection!");
        	if (g_label)
        	{
        		lv_obj_del(g_label);
        	    g_label = NULL;
            ...
            Demo_Display_Enable(true);
            //lv_label_set_text(label, "Motion Detected!");
            //lv_style_set_text_color(&style_title, lv_color_hex(0x00ff00));
            //lv_style_set_text_opa(&style_text_muted, LV_OPA_100);

            /* Dispaly motion detected 2s */
            lv_obj_t *motion_img = lv_img_create(lv_scr_act());
            LV_IMG_DECLARE(motion_detect);
            lv_img_set_src(motion_img, &motion_detect);
            lv_obj_center(motion_img);
            OSA_TimeDelay(3000);
			lv_obj_del(motion_img);

            lv_demo_widgets_input_pincode();
            break;
        case DIS_EVENT_FAR_DECT:
            lv_label_set_text(g_label, "Far Detection!");
            lv_style_set_text_color(&style_title, lv_color_hex(0x0000ff));
            break;
        case DIS_EVENT_WIFI_INIT:
        	screen = lv_scr_act();
        	if (!g_label)
        		g_label = lv_label_create(screen);
        	lv_obj_add_style(g_label, &style_title, 0);
        	lv_label_set_text(g_label, "Connecting AP!");
            lv_style_set_text_color(&style_title, lv_color_hex(0xEB5C20));
            lv_obj_align_to(g_label, NULL, LV_ALIGN_CENTER, 0, 0);
            break;
        default:
            break;
    }
