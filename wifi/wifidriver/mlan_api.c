...
void set_csi_filter_1(unsigned int *headerBuffer, hal_wls_packet_params_t *packetparams)
{
	hal_csirxinfo_t *csirxinfo = (hal_csirxinfo_t*)headerBuffer;
...
int check_csi_filter_1(unsigned int *headerBuffer, hal_wls_packet_params_t *packetparams)
{
	hal_csirxinfo_t *csirxinfo = (hal_csirxinfo_t*)headerBuffer;
	hal_pktinfo_t *pktinfo;
	unsigned int tempVec[2] = {0, 0};
...
//Bob++
#define PI_NEAR_THRESHOLD	-10 //-20
#define PI_FAR_THRESHOLD 	-15
#define PI_NUM_AVG			2 //5

extern void display_csi_sensing(uint8_t state);
//Bob++

void proc_csi_event(event_header * event)
{
	unsigned int resArray[8];
	unsigned char* rdPtr = (unsigned char*) event;
	static unsigned int *csiBuffer = NULL;
	...
	static unsigned int pi_count;
	static float last_pi = 0.0f;
	static unsigned int last_dis_event = 0xFF;
	...
	{
		PRINTF("csiBuffer is NULL\r\n");
		goto event_error;
	}

	packetparams.chNum = gwls_csi_cfg.channel;
	//DEBUG_TIME_US1=PLATFORM_GetTimeStamp();
	wls_unpack_csi(csiBuffer, fftInBuffer, &packetparams, &gwls_csi_cfg.wls_processing_input, totalpower);
	
	firstPathDelay = wls_calculate_toa(headerBuffer, fftInBuffer, scratchBuffer1, totalpower, 
			&packetparams, &gwls_csi_cfg.wls_processing_input);
	...
	{	// initialize
		...
		set_csi_filter_1(headerBuffer, &packetparams);
	}
	else if (check_csi_filter_1(headerBuffer, &packetparams))
	{
		perturbVal_dB = wls_update_cross_corr_ami_calc(headerBuffer, &g_ami_cfg.gcsi_filter_param, fftInBuffer, referenceBuffer, scratchBuffer1);
	
		{
			...
			//Bob++
			if (last_pi > PI_NEAR_THRESHOLD)
			{
				if (perturbVal_dB > PI_NEAR_THRESHOLD)
				{
					pi_count++;
					//PRINTF("NEAR count [%d]\r\n", pi_count);
				}
				...
				if((pi_count >= PI_NUM_AVG) && (last_dis_event != DIS_EVENT_NEAR_DECT))
				{
					PRINTF("Wakeup Display Task NEAR\r\n");
					display_csi_sensing(DIS_EVENT_NEAR_DECT);
					pi_count = 0;
					last_dis_event = DIS_EVENT_NEAR_DECT;
					//OSA_TimeDelay(50);
				}
			}
		#if 0
			if ((last_pi < PI_NEAR_THRESHOLD) & (last_pi > PI_FAR_THRESHOLD))
			{
				if ((perturbVal_dB < PI_NEAR_THRESHOLD) & (perturbVal_dB > PI_FAR_THRESHOLD))
				{
					pi_count++;
					//PRINTF("FAR count [%d]\r\n", pi_count);
				}
				...
				if((pi_count >= PI_NUM_AVG) && (last_dis_event != DIS_EVENT_FAR_DECT))
				{
					//PRINTF("Wakeup Display Task FAR\r\n");
					display_csi_sensing(DIS_EVENT_FAR_DECT);
					pi_count = 0;
					last_dis_event = DIS_EVENT_FAR_DECT;
					//OSA_TimeDelay(50);
				}
			}

			if (last_pi < PI_FAR_THRESHOLD)
			{
				if (perturbVal_dB < PI_FAR_THRESHOLD)
				{
					pi_count++;
				}
				else
				...
				if((pi_count >= PI_NUM_AVG) && (last_dis_event != DIS_EVENT_SENSING))
				{
					//PRINTF("Wakeup Display Task Sensing\r\n");
					display_csi_sensing(DIS_EVENT_SENSING);
					pi_count = 0;
					last_dis_event = DIS_EVENT_SENSING;
					//OSA_TimeDelay(50);
				}
			}
		#else
			...
			if (pi_count > PI_NUM_AVG)
				pi_count = 0;

			last_pi = perturbVal_dB;
		}
	}
